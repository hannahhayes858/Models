...
#### Q1.4: We predict that higher variability would be associated with larger crabs. Because generally we see larger species of crab in colder environments that experience more disterbance.
...
# Make the y-axis include 0
   ylim(0, NA) +
  labs(x = "Water Temperature SD", y = "Size (mm)", color = "Water Temperature (C)")
```
...
Since the Rhat value is equal to 1 we feel very confident that the model ran correctly. Since all of the chains line up and are in a straight line across we feel confident that the model ran correctly.
